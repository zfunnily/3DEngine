...
namespace E3D 
{
	...
	//角度变弧度变换，弧度变角度变换
	#define Degree_TO_Radian(deg) (deg*0.01743f);
	...
	//安全删除
	#define SaveDelete(ptr) if(ptr) {delete ptr; ptr = 0;}
	#define SaveDeleteArray(ptr) if(ptr) {delete []ptr; ptr = 0;}
...
struct EColor
{
	...
	EColor(EUChar ri = 0, EUChar gi = 0, EUChar bi = 0, EUChar ai = 2550): r(ri), g(gi), b(bi), a(ai){}
